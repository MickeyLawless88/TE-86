...
void Loop(void)
{
    ...
    while (run) {
        ...
        switch (ch) {
            ...
            /* Left-click: move cursor only, no auto-selection */
            case K_MOUSE_GOTO:
            {
            ...
            /* No longer generated by MousePoll but must compile */
            case K_MOUSE_BLK_END:
            case K_MOUSE_LINE_SEL:
            case K_MOUSE_PASTE:
                break;

            /* ---- Context menu: right-click or Ctrl+R ---- */
            case K_MOUSE_CTX:
            case K_CTX_MENU:
            {
                int menu_col, menu_row, action;

                if (ch == K_MOUSE_CTX) {
                    int mc, mr;
                    /* Wait for right button release before drawing the
                       menu, otherwise the held button instantly cancels */
                    while (MouseGetState(&mc, &mr) & 2) {}
                    menu_col = ms_last_click_col;
                    menu_row = ms_last_click_row;
                } else {
                    menu_col = cf_gutter + box_shc + 1;
                    menu_row = BOX_ROW + box_shr;
                }

                action = CtxMenu(menu_col, menu_row);

                if (action == K_COPY || action == K_CUT) {
                    if (clipboard) { free(clipboard); clipboard = NULL; }
#if OPT_BLOCK
                    if (blk_count > 0) {
                        clipboard = (char *)malloc(
                                        strlen(lp_arr[blk_start]) + 1);
                        if (clipboard)
                            strcpy(clipboard, lp_arr[blk_start]);
                        if (action == K_CUT) {
                            int first = blk_start;
                            ...
                            for (i = 0; i < cnt; i++)
                                DeleteLine(first);
                            if (first >= lp_now) first = lp_now - 1;
                            if (first < 0)       first = 0;
                            lp_cur = first;
                            blk_start = blk_end = -1;
                            blk_count = 0;
