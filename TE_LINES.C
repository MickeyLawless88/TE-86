/*  te_lines.c
    ...
    te-86 Text Editor - Line buffer operations.
...
int SplitLine(int line, int pos)
{
    ...
    /* Copy the right portion into a fresh buffer BEFORE AppendLine.
       AppendLine->SetLine shifts lp_arr[], making lp_arr[line+1]
       point at orig; then truncating orig at pos would corrupt the
       new line.  The temp copy avoids that aliasing.               */
    rlen = (int)strlen(orig + pos);
    if (!(tmp = AllocMem((unsigned int)(rlen + 1))))
        return 0;
    strcpy(tmp, orig + pos);

    if (AppendLine(line, tmp)) {
        lp_arr[line][pos] = '\0';
        free(tmp);
        return 1;
    }
