/*  tecf.c
    ...
    te-86 Configuration Tool for MS-DOS / Turbo C.

    Author: Mickey W. Lawless
    Date:   03/01/26

    Based on the CP/M version: Copyright (c) 2021 Miguel Garcia / FloppySoftware
    MS-DOS Turbo C port: 2024

    Usage:
        tecf patch [filename[.EXE] [filename[.CF]]]
        tecf dump  [filename[.EXE]]
        tecf key

    This program patches the configuration directly at byte offsets
    to avoid struct alignment issues between TECF and TE.
*/
