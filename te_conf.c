...
int CfSave(void)
{
    ...
    if (fwrite(cf_data, 1, 141, fp) != 141) {
        fclose(fp);
        return -1;
    }
...
int CfLoad(void)
{
    ...
    /* Load v3 fields only when the file is large enough.
       Older v2 CFG files simply keep the compiled-in defaults. */
    if (n >= 141) {
